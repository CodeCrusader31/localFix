...
//           <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 to-blue-500 opacity-0 group-hover:opacity-20 transition-opacity duration-500 rounded-full"></div>

//           {/* Subtle pulse effect */}
//           <div className="absolute inset-0 animate-pulse-slow opacity-0 group-hover:opacity-50 transition-opacity duration-1000 rounded-full">
//             <div className="absolute top-0 left-0 w-full h-full bg-white bg-opacity-10 rounded-full"></div>
//           </div>
//         </button>
//       </div>
//     </header>
...
const Header = ({ onSearch }) => {
  ...
  const handleSearch = (e) => {
    ...
    if (!searchTerm.trim()) {
      ...
      // If there's an onSearch prop, call it with empty string to clear search
      if (onSearch) {
        onSearch("");
      }
      return;
    ...
    // Call the onSearch prop if provided (to pass search term to parent)
    if (onSearch) {
      onSearch(searchTerm);
    }
    
    // You can also navigate to a search results page or perform other actions
    // router.push(`/services?search=${encodeURIComponent(searchTerm)}`);
  };

  // Handle real-time search as user types (optional)
  const handleInputChange = (e) => {
    const value = e.target.value;
    setSearchTerm(value);
    
    // For real-time search, uncomment below
    // if (onSearch) {
    //   onSearch(value);
    // }
  };
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        >
          <div className="relative flex-1">
            <input
              type="text"
              placeholder="Search for local services (e.g., Cleaning, Plumbing, Repair)..."
              value={searchTerm}
            ...
            {searchTerm && (
              <button
                type="button"
                onClick={clearSearch}
                className="absolute right-12 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400 hover:text-gray-600"
              >
                ✕
              </button>
            )}
